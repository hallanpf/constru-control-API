...
public class SecurityConfiguration {
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
    return httpSecurity.csrf(AbstractHttpConfigurer::disable).sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
            ...
            .requestMatchers(HttpMethod.POST, "/companies", "/constructions", "/employees", "/suppliers", "/brokers", "/manager").hasRole("GESTOR") // autorizando apenas o gestor a fazer requests nesses endpoints.
